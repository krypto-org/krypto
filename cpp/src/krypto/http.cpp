...
std::optional<std::string> krypto::HttpClient::post(const std::string& endpoint, const std::string& data,
        const std::unordered_map<std::string, std::string>& custom_headers) {
    KRYP_LOG(info, "Requesting {}:{}/{}", host_, port_, endpoint);
    boost::asio::io_context ioc;
    ssl::context ctx{ssl::context::sslv23_client};
    ...
    if (ec == boost::asio::error::eof) {
        KRYP_LOG(info, "EOF Received");
        ec.assign(0, ec.category());
    } else if (ec == boost::asio::ssl::error::stream_truncated) {
        KRYP_LOG(info, "Stream truncated");
    } else {
