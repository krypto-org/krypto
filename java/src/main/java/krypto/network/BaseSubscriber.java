...
public abstract class BaseSubscriber implements SocketMonitor.Sniffer {
    ...
    public BaseSubscriber(final ZMQ.Context context,
    ...
    public void stop() {
        ...
        if (subscriberThread.isAlive()) {
            try {
                ...
                LOGGER.error(
                        String.format("Error while joining the subscriber thread. Error Message: %s",
                                e.getMessage()));
            }
        ...
        if (monitor) {
            ...
            if (monitorThread.isAlive()) {
                try {
                    ...
                    LOGGER.error(
                            String.format("Error while joining the subscriber thread - %s",
                                    e.getMessage()));
                }
            }
        }
