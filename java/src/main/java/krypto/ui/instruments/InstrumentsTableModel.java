...
public class InstrumentsTableModel extends ReadOnlyTableModel {
  ...
  public String getColumnName(int column) {
    return InstrumentsTable.Column.values()[column].toString().replace("_", " ");
  }
  ...
  public Object getValueAt(int row, int column) {
    if (this.instrumentRowNums.containsKey(row)) {
      ...
      if (this.instruments.containsKey(instId)) {
        final InstrumentsTable.Column col = InstrumentsTable.Column.values()[column];
  ...
  public void reload() {
    this.instruments.putAll(this.dataCache.getInstruments(false));
    int row = 0;
