...
public class ModelFitChart extends ApplicationFrame {
  private final XYSeries marketSeries;
  private final XYSeries fittedSeries;
  private final XYSeriesCollection marketDataset;
  private final XYSeriesCollection fittedDataset;
  private Map<Double, Double> marketData = new HashMap<>();
  private Map<Double, Double> fittedData = new HashMap<>();

  public ModelFitChart(
      JPanel panel,
      String chartTitle,
      String scatterSeriesTitle,
    ...
    JFreeChart chart = createChart(chartTitle, xLabel, yLabel);
  ...
  private JFreeChart createChart(String chartTitle, String xTitle, String yTitle) {
    final JFreeChart result =
        ChartFactory.createScatterPlot(
            chartTitle, xTitle, yTitle, marketDataset, PlotOrientation.VERTICAL, true, true, false);

    final XYPlot plot = result.getXYPlot();
    plot.setDataset(0, marketDataset);
    plot.setBackgroundPaint(Color.DARK_GRAY);
    plot.setRangeGridlinePaint(Color.WHITE);
    plot.getRenderer().setSeriesPaint(0, ColorConstants.CustomColors.APPLICATION_GOLDEN);
