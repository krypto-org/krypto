...
public class TheoreticalSnapshotTableModel extends ReadOnlyTableModel {
  ...
  public TheoreticalSnapshotTableModel(
      final UIDataCache uiDataCache, final HeatmapColumnTableCellRenderer theoCellRenderer) {
  ...
  public Object getValueAt(int row, int column) {
    if (this.instrumentIdToRowNums.containsKey(row)) {
      final long instId = this.instrumentIdToRowNums.get(row);
      final TheoreticalSnapshotTable.Column col = TheoreticalSnapshotTable.Column.values()[column];
      if (col == TheoreticalSnapshotTable.Column.INSTRUMENT) {
        return this.uiDataCache.getActiveInstruments(false).get(instId).symbol();
      }
      if (this.uiDataCache.getTheos().containsKey(instId)) {
        final TheoreticalSnapshot snapshot = this.uiDataCache.getTheos().get(instId);
        switch (col) {
          case THEO:
            return DisplayConstants.PRICE_FORMAT.format(snapshot.price());
          case MM_BID:
            return DisplayConstants.PRICE_FORMAT.format(snapshot.mmBaseBid());
          case MM_ASK:
            return DisplayConstants.PRICE_FORMAT.format(snapshot.mmBaseAsk());
          case ERROR:
            return DisplayConstants.PRICE_FORMAT.format(snapshot.error());
          case BID_LIQUIDITY:
          ...
          case ADJUSTED_THEO:
            return DisplayConstants.PRICE_FORMAT.format(snapshot.adjustedPrice());
          case WIDTH_MULT:
  ...
  public void reinitialize() {
    int row = 0;
    for (final Instrument inst : this.uiDataCache.getInstruments(false).values()) {
      if (inst.active()) {
