...
public class StartScreen extends LiveFrame {
  ...
  public StartScreen(final UIDataCache uiDataCache, final OrderClient orderClient) {
    ...
    this.setIconImage(
        Toolkit.getDefaultToolkit().getImage(StartScreen.class.getResource(APPLICATION_ICON_PATH)));
    ...
    orderClient.registerListener(this.uiDataCache);
    ...
    Thread instrumentsCallThread =
        new Thread(
            () -> {
              ...
              this.instruments.putAll(this.uiDataCache.getActiveInstruments(true));
